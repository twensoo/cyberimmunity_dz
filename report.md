...
| **11. Система координирования транспорта**    | Модуль, получающий данные от (13) и (8). Основная координация  |     |
| **12.GPS**                                    | Модуль, получающий сигнал спутника                             |     |
| **13.Модуль маршрутизирования**               | Модуль, отвечающий за маршурт транспорта                       |     |
...
|-----------------------------------------------|---------------------------------------------|------------------------------------------|
| **1.Связь**                                   |🌕 доверенный, повышающий целостность данных | Этот компонент отвечает за распределение задач транспорта, критичен для ЦБ(Внутри содержатся группы доверенных и недоверенных компонентов, связанных друг с другом, по итогу взаимодействия которых *на выход идут достоверные данные*) |
...
| **4.Самодиагностика**                        |🔴 недоверенный                              | Компонент, который влияет на "старт" работы, не критичиен для наших ЦБ |
| **5.Контроль дорожной обстановки**           |🌕 доверенный, повышающий целостность данных | Выдает верные данные обстановки, критичен для ЦБ-3         |
| **6.Управление перемещением**                |🟢 доверенный                                | Осущетвляет перемещение автобуса, получает только достоверные данные, критичен для наших ЦБ |
...
| **10.Приводы**                               |🌕 доверенный, повышающий целостность данных | Влияет на осуществление перевозки пассажиров, критичен для ЦБ. (Внутри содержатся группы доверенных и недоверенных компонентов, связанных друг с другом, по итогу взаимодействия которых *на выход идут достоверные данные*) |
| **11.Система координирования транспорта**    |🌕 доверенный, повышающий целостность данных | Анализ, полученных данных. Получает данные из двух источников, анализируя, и тщательно проверяя - выдает достоверные данные для (6), влияет на ЦБ-2,3 |
| **12.GPS**                                    |🔴 недоверенный                              | Возможно получение неверных данных спутника                |
| **13.Модуль маршрутизирования**               |🔴 недоверенный                              | Возможно получение недостоверной информации                |



### Важно !
#### Здесь расмотрена общая архитектура, где необходимо доработать компоненты (10) (1), разбив их на более детальные составляющие. Сложные компоненты (CL) можно разбить на мелкие "куски", минимизуря доверенную кодовую базу
